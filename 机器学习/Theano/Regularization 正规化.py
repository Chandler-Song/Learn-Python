...
cost = T.mean(T.square(l2.outputs - y)) + 0.1*((l1.W ** 2).sum() + (l2.W**2).sum())  # without regularization l2 regularization
# cost = T.mean(T.square(l2.outputs - y)) + 0.1*(abs(l1.W).sum() + abs(l2).W,sum())  # without regularization l2 regularization
